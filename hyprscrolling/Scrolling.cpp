...
void CScrollingLayout::loadConfig() {
    ...
    m_config.fullscreen_on_one    = *CConfigValue<Hyprlang::INT>("plugin:hyprscrolling:fullscreen_on_one_column");
...
SDispatchResult CScrollingLayout::swap(const std::string& dir) {
    ...
    auto ws = dataFor(g_pCompositor->m_lastMonitor->m_activeWorkspace);
    if (!wd || wd->column->windowDatas.size() == 0 || !ws)
    ...
    auto it = std::find(ws->columns.begin(), ws->columns.end(), wd->column);
    if (it != ws->columns.end()) {
        auto i = std::distance(ws->columns.begin(), it);
        if (i >= 0 && i + offset < ws->columns.size()) {
            // maybe focus the swapped out window? who knows.
            // g_pCompositor->focusWindow(wds->columns[i+offset]->windowDatas.front()->window.lock());
            std::swap(ws->columns[i], ws->columns[i + offset]);
            centerOrFit(ws, ws->columns[i + offset]);
            ws->recalculate();
        }
    }
    return {};
