...
void SColumnData::add(PHLWINDOW w) {
  for (auto &wd : windowDatas) {
    wd->windowSize *=
        (float)windowDatas.size() / (float)(windowDatas.size() + 1);
  }

  windowDatas.emplace_back(makeShared<SScrollingWindowData>(
      w, self.lock(), 1.F / (float)(windowDatas.size() + 1)));
}
...
bool SColumnData::has(PHLWINDOW w) {
  return std::ranges::find_if(windowDatas, [w](const auto &e) {
           return e->window == w;
         }) != windowDatas.end();
}

SP<SColumnData> SWorkspaceData::add() {
  static const auto PCOLWIDTH =
      CConfigValue<Hyprlang::FLOAT>("plugin:hyprscrolling:column_width");
...
void SWorkspaceData::remove(SP<SColumnData> c) { std::erase(columns, c); }
...
void SWorkspaceData::centerCol(SP<SColumnData> c) {
  ...
  static const auto PFSONONE = CConfigValue<Hyprlang::INT>(
  ...
  for (const auto &COL : columns) {
    const double ITEM_WIDTH = *PFSONONE && columns.size() == 1
                                  ? USABLE.w
                                  : USABLE.w * COL->columnWidth;
...
void SWorkspaceData::fitCol(SP<SColumnData> c) {
  ...
  for (const auto &COL : columns) {
    ...
    if (COL != c)
      ...
      leftOffset = std::clamp((double)leftOffset,
                              currentLeft - USABLE.w + ITEM_WIDTH, currentLeft);
      return;
    }
  }
}
...
void SWorkspaceData::recalculate() {
  ...
  double currentLeft = 0;
  const double cameraLeft = MAX_WIDTH < USABLE.w
                                ? std::round((MAX_WIDTH - USABLE.w) / 2.0)
                                : leftOffset; // layout pixels
  ...
  for (const auto &COL : columns) {
    ...
    for (const auto &WINDOW : COL->windowDatas) {
      WINDOW->layoutBox =
          CBox{currentLeft, currentTop, ITEM_WIDTH,
               WINDOW->windowSize * USABLE.h}
              .translate(PMONITOR->m_position + PMONITOR->m_reservedTopLeft +
  ...
  for (const auto &col : columns) {
    if (col == c)
      return (totalLeft >= leftOffset && totalLeft < leftOffset + USABLE.w) ||
             (totalLeft + col->columnWidth * USABLE.w >= leftOffset &&
              totalLeft + col->columnWidth * USABLE.w < leftOffset + USABLE.w);
...
void CScrollingLayout::applyNodeDataToWindow(SP<SScrollingWindowData> data,
                                             bool force) {
  PHLMONITOR PMONITOR;
  ...
  // for gaps outer
  const bool DISPLAYLEFT =
      STICKS(data->layoutBox.x,
             PMONITOR->m_position.x + PMONITOR->m_reservedTopLeft.x);
  const bool DISPLAYRIGHT = STICKS(data->layoutBox.x + data->layoutBox.w,
                                   PMONITOR->m_position.x + PMONITOR->m_size.x -
                                       PMONITOR->m_reservedBottomRight.x);
  const bool DISPLAYTOP =
      STICKS(data->layoutBox.y,
             PMONITOR->m_position.y + PMONITOR->m_reservedTopLeft.y);
  const bool DISPLAYBOTTOM =
      STICKS(data->layoutBox.y + data->layoutBox.h,
  ...
  const auto OFFSETTOPLEFT =
      Vector2D((double)(DISPLAYLEFT ? gapsOut.m_left : gapsIn.m_left),
               (double)(DISPLAYTOP ? gapsOut.m_top : gapsIn.m_top));

  const auto OFFSETBOTTOMRIGHT =
      Vector2D((double)(DISPLAYRIGHT ? gapsOut.m_right : gapsIn.m_right),
               (double)(DISPLAYBOTTOM ? gapsOut.m_bottom : gapsIn.m_bottom));
  ...
  if (PWINDOW->m_isPseudotiled) {
    ...
    if (PWINDOW->m_pseudoSize.x > calcSize.x ||
        PWINDOW->m_pseudoSize.y > calcSize.y) {
      if (PWINDOW->m_pseudoSize.x > calcSize.x) {
  ...
  if (PWINDOW->onSpecialWorkspace() && !PWINDOW->isFullscreen()) {
    ...
    static auto PSCALEFACTOR =
        CConfigValue<Hyprlang::FLOAT>("dwindle:special_scale_factor");

    CBox wb = {calcPos + (calcSize - calcSize * *PSCALEFACTOR) / 2.f,
               calcSize * *PSCALEFACTOR};
...
void CScrollingLayout::onEnable() {
  m_configCallback = g_pHookSystem->hookDynamic(
      "configReloaded", [this](void *handle, SCallbackInfo &info,
                               std::any param) { this->loadConfig(); });
...
void CScrollingLayout::loadConfig() {
  ...
  m_config.focus_fit_method =
      *CConfigValue<Hyprlang::INT>("plugin:hyprscrolling:focus_fit_method");
  m_config.special_scale_factor =
      *CConfigValue<Hyprlang::FLOAT>("dwindle:special_scale_factor");
  m_config.fullscreen_on_one = *CConfigValue<Hyprlang::INT>(
      "plugin:hyprscrolling:fullscreen_on_one_column");
  m_config.center_on_focus =
      *CConfigValue<Hyprlang::INT>("plugin:hyprscrolling:center_on_focus");
  static const auto PCONFWIDTHS = CConfigValue<Hyprlang::STRING>(
      "plugin:hyprscrolling:explicit_column_widths");
  CConstVarList widths(*PCONFWIDTHS, 0, ',');
  for (auto &w : widths) {
    try {
      m_config.configuredWidths.emplace_back(std::stof(std::string{w}));
    } catch (...) {
      Debug::log(ERR, "scrolling: Failed to parse width {} as float", w);
    }
  }
}

void CScrollingLayout::onWindowCreatedTiling(PHLWINDOW window,
                                             eDirection direction) {
  auto workspaceData = dataFor(window->m_workspace);

  if (!workspaceData) {
    Debug::log(LOG, "[scrolling] No workspace data yet, creating");
    workspaceData = m_workspaceDatas.emplace_back(
        makeShared<SWorkspaceData>(window->m_workspace, this));
  ...
  if (droppingOn == window)
    droppingOn = g_pCompositor->vectorToWindowUnified(
        g_pInputManager->getMouseCoordsInternal(),
        RESERVED_EXTENTS | INPUT_EXTENTS);

  SP<SScrollingWindowData> droppingData =
      droppingOn ? dataFor(droppingOn) : nullptr;
  SP<SColumnData> droppingColumn =
      droppingData ? droppingData->column.lock() : nullptr;

  Debug::log(
      LOG,
      "[scrolling] new window {:x}, droppingColumn: {:x}, columns before: {}",
      (uintptr_t)window.get(), (uintptr_t)droppingColumn.get(),
...
void CScrollingLayout::onWindowRemovedTiling(PHLWINDOW window) {
  ...
  if (!DATA->column) {
    ...
    const auto USABLE = usableAreaFor(window->m_monitor.lock());
    WS->leftOffset = std::clamp((double)WS->leftOffset, 0.0,
                                std::max(WS->maxWidth() - USABLE.w, 1.0));
  }
}
...
void CScrollingLayout::resizeActiveWindow(const Vector2D &delta,
                                          eRectCorner corner,
                                          PHLWINDOW pWindow) {
  ...
  if (!DATA) {
    *PWINDOW->m_realSize =
        (PWINDOW->m_realSize->goal() + delta)
            .clamp(PWINDOW->m_windowData.minSize.valueOr(
                       Vector2D{MIN_WINDOW_SIZE, MIN_WINDOW_SIZE}),
                   PWINDOW->m_windowData.maxSize.valueOr(
  ...
  if (!DATA->column || !DATA->column->workspace ||
      !DATA->column->workspace->workspace ||
      !DATA->column->workspace->workspace->m_monitor)
    return;

  const auto USABLE =
      usableAreaFor(DATA->column->workspace->workspace->m_monitor.lock());
  ...
  case CORNER_TOPLEFT: {
    ...
    PREV_COLUMN->columnWidth =
        std::clamp(PREV_COLUMN->columnWidth + (float)DELTA_AS_PERC.x,
                   MIN_COLUMN_WIDTH, MAX_COLUMN_WIDTH);
    CURR_COLUMN->columnWidth =
        std::clamp(CURR_COLUMN->columnWidth - (float)DELTA_AS_PERC.x,
                   MIN_COLUMN_WIDTH, MAX_COLUMN_WIDTH);
    break;
  }
  ...
  case CORNER_TOPRIGHT: {
    ...
    NEXT_COLUMN->columnWidth =
        std::clamp(NEXT_COLUMN->columnWidth - (float)DELTA_AS_PERC.x,
                   MIN_COLUMN_WIDTH, MAX_COLUMN_WIDTH);
    CURR_COLUMN->columnWidth =
        std::clamp(CURR_COLUMN->columnWidth + (float)DELTA_AS_PERC.x,
                   MIN_COLUMN_WIDTH, MAX_COLUMN_WIDTH);
  ...
  if (DATA->column->windowDatas.size() > 1) {
    ...
    case CORNER_BOTTOMRIGHT: {
      ...
      float adjust = std::clamp((float)(delta.y / USABLE.h),
                                (-CURR_WD->windowSize + MIN_ROW_HEIGHT),
                                (NEXT_WD->windowSize - MIN_ROW_HEIGHT));

      NEXT_WD->windowSize = std::clamp(NEXT_WD->windowSize - adjust,
                                       MIN_ROW_HEIGHT, MAX_ROW_HEIGHT);
      CURR_WD->windowSize = std::clamp(CURR_WD->windowSize + adjust,
                                       MIN_ROW_HEIGHT, MAX_ROW_HEIGHT);
      break;
    }
    case CORNER_TOPLEFT:
    case CORNER_TOPRIGHT: {
      ...
      if (PREV_WD->windowSize <= MIN_ROW_HEIGHT && delta.y <= 0 ||
          CURR_WD->windowSize <= MIN_ROW_HEIGHT && delta.y >= 0)
        break;

      float adjust = std::clamp((float)(delta.y / USABLE.h),
                                -(PREV_WD->windowSize - MIN_ROW_HEIGHT),
                                (CURR_WD->windowSize - MIN_ROW_HEIGHT));

      PREV_WD->windowSize = std::clamp(PREV_WD->windowSize + adjust,
                                       MIN_ROW_HEIGHT, MAX_ROW_HEIGHT);
      CURR_WD->windowSize = std::clamp(CURR_WD->windowSize - adjust,
                                       MIN_ROW_HEIGHT, MAX_ROW_HEIGHT);
      break;
...
void CScrollingLayout::fullscreenRequestForWindow(
  ...
  if (EFFECTIVE_MODE == FSMODE_NONE) {
    ...
    if (EFFECTIVE_MODE == FSMODE_FULLSCREEN) {
      ...
      *pWindow->m_realSize = PMONITOR->m_size;
      ...
      fakeNode->layoutBox = {PMONITOR->m_position + PMONITOR->m_reservedTopLeft,
...
void CScrollingLayout::centerOrFit(const SP<SWorkspaceData> ws,
                                   const SP<SColumnData> col) {
  ...
  if (args[0] == "active") {
    ...
    if (!begun)
      return {.success = false,
              .error = "Failed to find any visible columns on workspace"};
    ws->leftOffset = 0;
    if (foundAt != 0) {
      const auto USABLE = usableAreaFor(ws->workspace->m_monitor.lock());
      for (size_t i = 0; i < foundAt; ++i) {
...
SDispatchResult CScrollingLayout::colresize(const std::string &str) {
  ...
  CScopeGuard x([w] {
    w->column->columnWidth =
        std::clamp(w->column->columnWidth, MIN_COLUMN_WIDTH, MAX_COLUMN_WIDTH);
    w->column->workspace->fitCol(w->column.lock());
    w->column->workspace->recalculate();
  });
  if (args[0][0] == '+' || args[0][0] == '-') {
    if (args[0] == "+conf") {
      auto it = std::upper_bound(m_config.configuredWidths.begin(),
                                 m_config.configuredWidths.end(),
                                 w->column->columnWidth);
      width = (it != m_config.configuredWidths.end())
                  ? *it
                  : m_config.configuredWidths[0];
    } else if (args[0] == "-conf") {
      auto it = std::lower_bound(m_config.configuredWidths.begin(),
                                 m_config.configuredWidths.end(),
                                 w->column->columnWidth);
      width = (it != m_config.configuredWidths.begin())
                  ? *(--it)
  ...
  if (width > 1.0 || width <= 0) {
    return {.success = false,
            .error = "Value larger than 1.0 is not supported at the moment."};
  }
...
SDispatchResult CScrollingLayout::focus(const std::string &arg) {
  ...
  default:
    return {.success = false,
            .error = "Invalid input. Valid inputs are: u,d,l,r"};
  }
  return {};
}
...
SDispatchResult CScrollingLayout::promote() {
  ...
  auto col =
      idx == -1 ? w->column->workspace->add() : w->column->workspace->add(idx);
...
std::any CScrollingLayout::layoutMessage(SLayoutMessageHeader header,
                                         std::string message) {
...
/* NOTE: Doesn't work like you'd expect. It switches with the last window
 * focused. So PHLWINDOW b is almost always m_lastWindow*/
void CScrollingLayout::switchWindows(PHLWINDOW a, PHLWINDOW b) {
  ...
  if ((col_a >= 0 && col_a < ws->columns.size()) &&
      (col_b >= 0 && col_b < ws->columns.size())) {
...
void CScrollingLayout::moveWindowTo(PHLWINDOW w, const std::string &dir,
                                    bool silent) {
...
CBox CScrollingLayout::usableAreaFor(PHLMONITOR m) {
  return CBox{m->m_reservedTopLeft,
              m->m_size - m->m_reservedTopLeft - m->m_reservedBottomRight};
}
